...
async def select_avoid_categories(user_prompt: str, found_categories: list[str]) -> tuple[list[str], str]:
    ...
    prompt = (
        ...
        "- infrastructure: считаем, что объекты категории infrastructure могут быть потенциально опасны (в рамках кейса "
        "с безопасностью для ребёнка). Поэтому: если в списке found_categories встречается infrastructure — включи "
        "infrastructure в avoid_categories. Исключение: если пользователь явно просит «не обходить» infrastructure "
        "(или что можно пройти через infrastructure), тогда не добавляй infrastructure.\n"
